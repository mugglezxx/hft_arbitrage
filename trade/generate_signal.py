...
async def order_book(book, receipt_timestamp):
    ...
    if nbbo_book[symbol_id]["NBBOMT"]["spread"] > nbbo_book[symbol_id]["NBBOMT"]["signal"]:
        ...
        if opm.order_database.hexists(maker_exchange, symbol_id) == True and opm.order_database.hexists(taker_exchange, symbol_id) == True:

            rm.q_new_order(symbol_id, buy_order, maker_exchange, "cancel")
            rm.q_new_order(symbol_id, sell_order, taker_exchange, "cancel")

        elif opm.order_database.hexists(maker_exchange, symbol_id) == True:

            rm.q_new_order(symbol_id, buy_order, maker_exchange, "cancel")
        
        elif opm.order_database.hexists(taker_exchange, symbol_id) == True:
